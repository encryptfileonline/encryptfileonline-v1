...
CipherDrop is an ultra-modern, minimalist web application designed for secure, client-side file encryption and decryption. Built with a strong focus on user experience and visual elegance, it allows users to encrypt files directly in their browser using the robust WebCrypto API (AES-256-GCM) and Argon2 for key derivation. The core philosophy is 'zero-trust,' meaning no files, passphrases, or keys ever leave the user's device.
...
-   **Strong Key Derivation**: User passphrases are converted into secure encryption keys using Argon2.
...
-   **Icons**: Lucide React
-   **Client-Side Cryptography**: WebCrypto API, Argon2
...
-   **Client-Side Only**: CipherDrop operates entirely within your browser. Your files and passphrases are never uploaded to any server.
